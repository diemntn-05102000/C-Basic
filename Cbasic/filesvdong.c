...
int main(int argc, char* argv[])
{
	...
	if (fptr2==NULL)
	{
	...
	else
	  {
		...
		printf("Nhap them so sv : ");
		scanf("%d",&n);
		//fptr2 = (FILE*)realloc(fptr2,n*sizeof(pepe));
		while( a<n)
		{
		  
		  printf("Nhap thong tin sv %d:",a+i+1);
		  scanf("%d %lu %s %s %d",&id[i+a].stt,&id[i+a].mssv,id[i+a].name,id[i+a].sdt,&id[i+a].diem);
		  printf("Success\n");
		  fprintf(fptr2,"%d %lu %s %s %d\n",id[i+a].stt,id[i+a].mssv,id[i+a].name,id[i+a].sdt,id[i+a].diem);
		  a++;
		}
	}
	fclose(fptr1);
